...
but_wide <- all_data_wkly %>%
  ...
  filter(study_week == "week1" | study_week == "week3", quantity_compliant == "yes", frequency == "1xdaily") %>% #keep only weeks of interest, quantity compliant, and 1x
...
delta_but_pro_wide_HMB <- inner_join(x = but_wide, y = pro_wide,
                    by = c("participant_id", 
                           "semester", "supplement_consumed")) %>%
  select(-starts_with("week"))
